...
usage() {
  cat <<'EOF'
Usage: unit_tests/sh2/test_campaign.sh [options]
...
Options:
  ...
  --skip-run                              Skip running emulator (requires existing unit_tests/sh2/uts.log)
  --log <path>                            Path to uts.log (default: <repo>/unit_tests/sh2/uts.log)
...
SKIP_RUN=0
STRICT=0
LOG_FILE="$ROOT_DIR/unit_tests/sh2/uts.log"
...
if [[ $SKIP_RUN -eq 0 ]]; then
  ...
  (
    cd "$ROOT_DIR/unit_tests/sh2"
    bash "$ROOT_DIR/unit_tests/sh2/run_tests.bat" "$EMULATOR"
  )
fi
